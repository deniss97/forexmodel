...
Прогнано на двух инструментах при реальной комиссии 0.04% за круг. LKOH: конфиг по
умолчанию прибылен на обеих независимых выборках (+3.84% на test при PF 1.53, +2.94% на
sim при PF 2.92), а каскад фильтров работает как задумано — чем строже отбор, тем лучше
сделка до комиссии. Серебро: 2025 год убыточен при любых настройках, а +64% на sim-2026 —
один пойманный обвал 30 января (две сделки дали половину результата). Лучший одиночный
сигнал проекта — нетто-дельта крупных сделок из ленты LKOH; как правило входа без модели
она даёт столько же, сколько CatBoost с 85 признаками, при вдвое меньшей просадке.
Разбор и что делать дальше — там же.

Конфигурации: [configs/default.yaml](configs/default.yaml) (LKOH),
[configs/silver.yaml](configs/silver.yaml) (серебро), и две пары для сравнения
«с признаками потока сделок / без» на периоде 2024–2026, где есть лента:
[lkoh_2024_base_reg.yaml](configs/lkoh_2024_base_reg.yaml) /
[lkoh_2024_orderflow_reg.yaml](configs/lkoh_2024_orderflow_reg.yaml) и
[silver_2024_base_reg.yaml](configs/silver_2024_base_reg.yaml) /
[silver_2024_orderflow_reg.yaml](configs/silver_2024_orderflow_reg.yaml).
На LKOH нетто-дельта крупных сделок (`net_large_delta`) — лучший одиночный признак
проекта (IC +0.084); на серебре, где лента — по фьючерсам MOEX, а котировки — спот
XAGUSD, эффект не воспроизвёлся (IC около нуля).

### Гейт входа: отдельный тренд-фильтр для симуляции

Тренд-фильтр `trend` даёт модели признаки `trend_4h` и т.д. и по умолчанию же решает, в какую
сторону разрешён вход. Чтобы поменять правило входа, не меняя признаков (и не переобучая модели),
есть секция `trend_gate` — её параметры переопределяют `trend`, а результат пишется в колонку
`trend_gate`, которая в модель не попадает:

```yaml
trend_gate:
  enabled: true
  require_adx_rising: false   # при развороте ADX сначала падает — новый тренд блокировался часами
  update_every_bar: true      # 4h-окна на каждой часовой сетке: обновление каждый час, а не раз в 4
  hold_bars: 2                # короткая нейтраль не сбрасывает направление (от мигания)
simulation:
...
data:
  ...
  orderflow_tz_shift_hours: 3                          # котировки — в московском (без DST)
  # orderflow_tz: America/New_York                     # либо IANA-зона котировок с учётом DST
features:
  ...
  orderflow_profile_bin: 10.0         # шаг ценового бина (в единицах цены потока)
  orderflow_same_instrument: true     # false, если лента по другому инструменту
                                      # (фьючерс против спота): ценовые признаки
                                      # тогда считаются в собственной шкале потока
```

Лента сделок может быть и по другому инструменту, чем котировки — так сделано для
серебра ([configs/silver_2024_orderflow_reg.yaml](configs/silver_2024_orderflow_reg.yaml)):
котировки — спот XAGUSD в нью-йоркском времени, поток сделок — фьючерсы SV* на MOEX
в UTC. Часовой пояс котировок определялся эмпирически: по совпадению уровней цены и
корреляции часовых доходностей фьючерса и спота при разных сдвигах. Если в ленте несколько
контрактов (колонка `TICKER_CC`), объёмные величины суммируются по всем, а ценовые
(close/high/low/VWAP бара и профиль объёма) берутся по фронт-контракту бара — контракту с
максимальным объёмом в нём: базис между одновременно торгующимися сроками у серебра на MOEX
5–9%, и смешивать их цены нельзя.
...
Скользящие окна считаются на собственных барах ленты (непрерывных в торговых часах),
а не на барах котировок: если котировки живут 24/5, а биржа — 15 часов в сутки, то
окно по барам котировок всегда захватывает ночной разрыв и целиком уходит в NaN.
Там, где потока сделок нет (ночь, выходные, до начала данных), признаки остаются NaN
(CatBoost это переваривает).
...
# варианты тренд-фильтра и правила входа (запрет против свечи, поздний вход, пауза после
# стопа): качество разметки тренда, проверка на истории без модели и на сигналах прогона
